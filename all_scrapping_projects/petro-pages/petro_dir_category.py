...
company_csv = open('petro_pages_main_category.csv', 'w')
csvwriter = csv.writer(company_csv)

# csvwriter.writerow('')
csvwriter.writerow(['Category_name','Category_ID','URL','friendlyURL'])

new_page = 'https://petropages.directory/'

page = requests.get(new_page)
time.sleep(0.5)
soup = BeautifulSoup(page.content, 'html.parser')
row_provider = soup.find(class_="entrylist").findAll('li')

print len(row_provider), type(row_provider)

for i in row_provider:
	company_list = []
	Category_name = i.find('a').get_text()
	Category_ID = i.a['category_id']
	URL = i.a['href']
	friendlyURL = i.a['friendlyurl']
	print Category_name, Category_ID, URL, friendlyURL

	company_list.append(Category_name.encode('ascii', 'ignore').decode('ascii'))
	company_list.append(Category_ID.encode('ascii', 'ignore').decode('ascii'))
	company_list.append(URL.encode('ascii', 'ignore').decode('ascii'))
	company_list.append(friendlyURL.encode('ascii', 'ignore').decode('ascii'))
